...
def store_kplets_pile(kplets_pile, profile2id, file2id):
    ...
    kplet_table_name = "crispr_3plets"
    kplet_table_file_name = "crispr_3plets_wgs"
    ...
    for (kplets, fname) in kplets_pile:
        ...
        for kplet in kplets:
            ...
            _sql_kplet_file += ("""((select id from crispr_3plets where """ +
...
def get_report_kplets(id2cdd, limit_to=500):
    ...
    _db.cmd = """select cp.*, count(distinct cwf.id) as cnt, group_concat(cwf.name) as files
                from crispr_3plets cp
                inner join crispr_3plets_wgs cpw on cp.id = cpw.kplet_id
