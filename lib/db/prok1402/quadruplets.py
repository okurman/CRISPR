...
def get_multiple_kplets():
    ...
    _db.cmd = """ select  ap.id, count(*) cnt, group_concat(convert(apw.file_id, char(15))) as file_ids
                  from bacteria_4plets ap
                  inner join bacteria_4plets_win10 apw on ap.id = apw.kplet_id
                  group by ap.id
                  having count(*)>1
                  order by cnt desc"""
...
def get_code_kplet(kplet_id, id2cdd=None):
    ...
    if not id2cdd:
        _db.cmd = """select cp1.code, cp2.code, cp3.code, cp4.code
                    from bacteria_4plets bp
                    ...
                    inner join cdd_profiles cp4 on cp4.id = bp.kplet_4
                    where bp.id = %d""" % kplet_id
        retval = _db.retrieve()[0]

    else:

        _db.cmd = """select kplet_1, kplet_2, kplet_3, kplet_4
                     from bacteria_4plets where id = %d""" % kplet_id
